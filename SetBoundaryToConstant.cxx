...
int main(int argc, char *argv[])
{
  ...
  std::cout << "Constant: " << constantInt << std::endl;
  ...
  while(!outputIterator.IsAtEnd())
    ...
    if(index[0] == 0 || static_cast<unsigned int>(index[0]) == output->GetLargestPossibleRegion().GetSize()[0] -1 ||
       index[1] == 0 || static_cast<unsigned int>(index[1]) == output->GetLargestPossibleRegion().GetSize()[1] -1)
      {
      outputIterator.Set(constantInt);
      }
